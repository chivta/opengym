...
- 🔑 **Passkeys first, password if you need one** — Face ID / Touch ID / fingerprint login, or a name and password on a device that can't make a passkey; one account can hold both. Each profile keeps its own data, synced across devices. Sign-ins last 90 days by default (configurable), and “sign out everywhere” in Settings ends every session on every device at once
